...
from pis_product.models import Outcaisse, PurchasedProduct, ExtraItems,StockOut, StockIn, Product, Category, Supplier, Itemsbysupplier, Avancesbon, Mark, PaymentSupplier, Avoirsupp, PaymentClient, Supplierprice, Clientprice, Returned, Outcaisseext, Outbank, Facture
...
def relveclient(request):
    ...
    bons = SalesHistory.objects.filter(customer_id=customer, datebon__range=[start_date, end_date], hasmultiple=False).order_by('datebon')
...
def bondata(request):
    ...
    bon=SalesHistory.objects.get(pk=id)
    #here
    items=PurchasedProduct.objects.filter(Q(invoice=bon)|Q(facture=bon))
...
def avoirdata(request):
    ...
    items=Returned.objects.filter(avoir=bon)
    return JsonResponse({
        'data':render(request, 'products/bondata.html', {'bon':bon, 'avoir':True, 'items':items}).content.decode('utf-8')
    })
def viewavoir(request):
    id=request.GET.get('id')
    avoir=Avoir.objects.get(pk=id)
    items=Returned.objects.filter(avoir=avoir)
    return render(request, 'products/updateavoir.html', {
        'avoir':avoir,
        'items':items,
        'title':'Modifier avoir'
    })

@csrf_exempt
def updateavoir(request):
    id=request.POST.get('id')
    avoir=Avoir.objects.get(pk=id)
    olditems=Returned.objects.filter(avoir=avoir)
    ...
    client=avoir.customer
    client.rest=float(client.rest)+float(avoir.grand_total)-float(total)
    client.save()
    for i in olditems:
        product=i.product
        product.stock-=i.qty
    ...
    with transaction.atomic():
        for i in items:
            print('>> itemsid', i.get('item_id'))
            print('>>', i, Product.objects.get(pk=i.get('item_id')))
            item=i.get('item_id')
            product = Product.objects.get(pk=item)
            # append product.prices
...
def addclient(request):
    ...
    client=Customer.objects.create(rest=0, customer_name=name,customer_phone=phone, address=address, ice=ice, retailer=Retailer.objects.get(id=request.user.retailer_user.retailer.id))
...
def producthistory(request, id):
    ...
    if not userprofile.canviewproduct:
        return render(request, 'products/nopermission.html')
    product=Product.objects.get(pk=id)
    pr=StockIn.objects.filter(product=product).order_by('-reciept__bondate')
    stockout=PurchasedProduct.objects.filter(
        Q(product=product) & (
            (Q(invoice__ismanual=False) )| 
            Q(isavoirsupp=True)
        )
    ).order_by('-created_at')
...
def reports(request):
    sales=round(PurchasedProduct.objects.filter(isavoirsupp=False, created_at__year=this_year, created_at__month=this_month
        ).aggregate(
        total_revenue=Sum('purchase_amount')
    )['total_revenue'] or 0, 2)
    avoircl=round(Avoir.objects.filter(dateavoir__year=this_year, dateavoir__month=this_month
        ).aggregate(
        total=Sum('grand_total')
    )['total'] or 0, 2)
    achats=round(StockIn.objects.filter(dated_order__year=this_year, dated_order__month=this_month
        ).aggregate(
            total_cost=Sum(('total'))
        )['total_cost'] or 0, 2)
    avoirsupp=round(Avoirsupp.objects.filter(date__year=this_year, date__month=this_month
        ).aggregate(
        total=Sum('total')
    )['total'] or 0, 2)
    one_year_ago = timezone.now().date() - timedelta(days=3*366)
    sold_product_ids = PurchasedProduct.objects.filter(isavoirsupp=False, invoice__datebon__gte=one_year_ago
    ).values_list('product_id', flat=True).distinct()

    # Products never sold in the past year
    unsold_products = Product.objects.filter(stock__gt=0).exclude(id__in=sold_product_ids)    
    print(">> unsold_products", unsold_products)
    ...
    return render(request, 'products/reports.html', ctx)
...
def reportnetprofit(request):
    ...
    sales=round(PurchasedProduct.objects.filter(isavoirsupp=False, invoice__datebon=date
        ).aggregate(
        total_revenue=Sum('purchase_amount')
    )['total_revenue'] or 0, 2)
    avoircl=round(Avoir.objects.filter(dateavoir=date
        ).aggregate(
        total=Sum('grand_total')
    )['total'] or 0, 2)
    achats=round(StockIn.objects.filter(dated_order=date
        ).aggregate(
            total_cost=Sum(('total'))
        )['total_cost'] or 0, 2)
    avoirsupp=round(Avoirsupp.objects.filter(date=date
        ).aggregate(
        total=Sum('total')
    )['total'] or 0, 2)
    ventes=PurchasedProduct.objects.filter(isavoirsupp=False, invoice__datebon=date)
    trs=''
    totalmarge=0
    for  i in ventes:
        diff=i.price-i.product.prnet
...
def clientsranking(request):
    pass
    # top_customers = (SalesHistory.objects
    # .filter(customer__isnull=False)  # Ensure customer exists
    # .values('customer')
    # .annotate(total_sales=Sum('grand_total'))
    # .order_by('-total_sales')[:10])
    # return render(request, 'products/productsranking.html', {'products':products}).content.decode('utf-8')
...
def updatebonachat(request, id):
    ...
    last_reciept = Itemsbysupplier.objects.exclude(pk=bon.pk).order_by('-bondate').first()
    if request.method=="POST":
        nbon=request.POST.get('nbon')
        bondate=request.POST.get('bondate')
        bondate=datetime.strptime(bondate, '%Y-%m-%d')
        ...
        for i in items:
            ...
            lastentry=StockIn.objects.filter(product=product, avoir_reciept=None).order_by('-reciept__bondate').exclude(reciept=bon).first()
            islastentry= False 
            if lastentry.reciept:
                if lastentry.reciept.bondate<=bondate:
                    islastentry=True
            else:
                islastentry=True
            print('>> islast', islastentry)
            if islastentry:
...
def addsupply(request):
    ...
    with transaction.atomic():
        for i in items:
            ...
            lastentry=StockIn.objects.filter(product=product, avoir_reciept=None).order_by('-reciept__bondate').first()
            islastentry= False 
            if lastentry.reciept:
                if lastentry.reciept.bondate<=bondate:
                    islastentry=True
            ...
            prices.append([f'{supplier.name} - {reciept.nbon}', str(bondate), float(i['price']), float(i['qty'])])
            # qts=0
            # prcs=0
            # for b in prices:
            #     qts+=b[3]
...
def supplierslist(request):
    ...
    if not userprofile.canseesuppliers:
        return render(request, 'products/nopermission.html')
    suppliers=Supplier.objects.all().order_by('-rest')
    # supplier_data = []
...
def supplierinfo(request, id):
    ...
    if not userprofile.canseesupplier:
        return render(request, 'products/nopermission.html')
    supplier=Supplier.objects.get(pk=id)
    payments=PaymentSupplier.objects.filter(supplier=supplier).order_by('-echeance')
    nbrpayments=payments.count()
    totalpayments=payments.aggregate(total=Sum('amount'))['total'] or 0
    bons=Itemsbysupplier.objects.filter(supplier=supplier).order_by('-date')
    avoirs=Avoirsupp.objects.filter(supplier=supplier).order_by('-date')
...
def getclientprice(request):
    ...
    clientprice=PurchasedProduct.objects.filter(invoice__customer_id=clientid, product_id=productid).last()
    if clientprice:
        return JsonResponse({
...
def echeances(request):
    ...
    echeances=PaymentSupplier.objects.filter(Q(mode="echeanceEspece")|Q(mode="effet")| Q(mode="cheque"), echeance__month=this_month, echeance__year=this_year).order_by("echeance")
    espece=echeances.filter(iscash=True).aggregate(Sum('amount')).get('amount__sum') or 0
    bank=echeances.filter(ispaid=True).aggregate(Sum('amount')).get('amount__sum') or 0
    totalamount=round(echeances.aggregate(Sum('amount'))['amount__sum'] or 0, 2)
    totalpaid=round(echeances.filter(Q(ispaid=True)|Q(iscash=True)).aggregate(Sum('amount'))['amount__sum'] or 0, 2)
    rest=round(totalamount-totalpaid, 2)
    today=date.today()
...
def factureprint(request, id):
    footer=request.GET.get('footer')=='1'
    order=SalesHistory.objects.get(pk=id)
    if order.hasmultiple:
        orderitems=PurchasedProduct.objects.filter(facture=order)
    else:
...
def listfactures(request):
    ctx={
        'title':'List factures',
        'bons':SalesHistory.objects.filter(Q(ismanual=True) | Q(isfacture=True) | Q(hasmultiple=True)).order_by('-datebon')
    }
...
def relvebank(request):
    ...
    end_date = datetime.strptime(end_date_str[:10], '%d/%m/%Y') + timedelta(days=1)
    print(start_date, end_date)
    outs=Outbank.objects.filter(date__range=[start_date, end_date])
    outspaymentsuppp=PaymentSupplier.objects.filter(mode="verment", date__range=[start_date, end_date])
    if outspaymentsuppp.count() > 0:
        totalout=round(float(outs.aggregate(Sum('amount'))['amount__sum'] or 0), 2)+round(float(outspaymentsuppp.aggregate(Sum('amount'))['amount__sum'] or 0), 2)
    else:
        totalout=round(float(outs.aggregate(Sum('amount'))['amount__sum'] or 0), 2)+0

    ins=Outcaisseext.objects.filter(bank=True, date__range=[start_date, end_date])
    insclientpayments=PaymentClient.objects.filter(mode="verment", date__range=[start_date, end_date])
    if insclientpayments.count() > 0:
        totalin=round(float(ins.aggregate(Sum('amount'))['amount__sum']or 0) , 2)+round(float(insclientpayments.aggregate(Sum('amount'))['amount__sum']or 0), 2)
    else:
        totalin=round(float(ins.aggregate(Sum('amount'))['amount__sum']) or 0, 2)+0
    ...
    print('sold >>', (request.user.retailer_user.retailer.bank or 0)-2000)
    # Sort the items by date
    releve = sorted(releve, key=lambda item: item[0].date)
    ctx={
        'title':'Releve compte banquaire',
        'releve':releve,
        'sold':float(totalin)-float(totalout),
    ...
    return render(request, 'products/relvebank.html', ctx)
def getecheancesdate(request):
    ...
    # datefrom = datetime.strptime(datefrom, date_format)
    # dateto = datetime.strptime(dateto, date_format)
    echeances= PaymentSupplier.objects.filter(
        Q(mode__in=["echeanceEspece", "effet", "cheque"]),
        echeance__month=month,
        echeance__year=year
    )
    print('>>', echeances.count())
    total=round(echeances.aggregate(Sum('amount')).get('amount__sum') or 0, 2)
    espece=round(echeances.filter(iscash=True).aggregate(Sum('amount')).get('amount__sum') or 0, 2)
    bank=round(echeances.filter(ispaid=True).aggregate(Sum('amount')).get('amount__sum') or 0, 2)
    totalamount=round(echeances.aggregate(Sum('amount'))['amount__sum'] or 0, 2)
    totalpaid=round(echeances.filter(Q(ispaid=True)|Q(iscash=True)).aggregate(Sum('amount'))['amount__sum'] or 0, 2)
    rest=round(totalamount-totalpaid, 2)
    today=date.today()
    ...
    ctx={
        ...
        'today':today,
        'title':'Echeances',
        'echeances':echeances.order_by('echeance'),
        'totalamount':total,
        
    }
    return JsonResponse({
        'trs':render(request, 'products/echeancestrs.html',ctx).content.decode('utf-8'),
        'total':total,
        'espece':espece,
        'bank':bank,
        'totalpaid':totalpaid,
...
def printcommand(request):
    supplierid=request.GET.get('supplierid')
    products=Product.objects.filter(supplier_id=supplierid)
    ctx={
        'products':products,
        'supplier':Supplier.objects.get(pk=supplierid),
        'today':timezone.now()
    }
    return render(request, 'products/printcommand.html', ctx)
...
def facturation(request):
    ...
    bons = SalesHistory.objects.filter(
        isfacture=False,
        ismanual=False,
        inmultiple=False,
        hasmultiple=False,
        datebon__range=[datefrom, dateto],
        customer_id=clientid
    )
    
    if len(bons)==0:
        return JsonResponse({'success': False, 'message':'Aucun bon dans les dates specifié'})
    items = PurchasedProduct.objects.filter(invoice__in=bons)

    total = round(bons.aggregate(Sum('grand_total')).get('grand_total__sum') or 0, 2)
    print('>>> len(bons)f', len(bons), len(items))
    year = timezone.now().strftime("%y")
    latest_receipt = SalesHistory.objects.filter(
        receipt_no__startswith=f'FC{year}'
    ).order_by('-receipt_no').first()

    if latest_receipt:
        latest_receipt_no = int(latest_receipt.receipt_no[-9:])
        receipt_no = f"FC{year}{latest_receipt_no + 1:09}"
    else:
        receipt_no = f"FC{year}000000001"
    # bon numbers to put therm in note facture
    numbers=' '.join(i.receipt_no for i in bons)
    bons.update(inmultiple=True)
    fc = SalesHistory.objects.create(
        customer_id=clientid,
    ...
    bons.update(note=f'Facture N° {receipt_no}')
...
def deleteavoirclient(request):
    ...
    for i in items:
        ...
        product.stock-=i.qty
        product.save()
    clientid=avoir.customer.id
    client=Customer.objects.get(pk=clientid)
    client.rest+=avoir.grand_total
    client.save()
    avoir.delete()
    return redirect('ledger:customer_ledger_detail', customer_id=clientid)

def deleteavoirsupp(request):
    id=request.GET.get('id')
    avoir=Avoirsupp.objects.get(pk=id)
    suppid=avoir.supplier.id
    supplier=Supplier.objects.get(pk=suppid)
    supplier.rest+=avoir.total
    supplier.save()
    ...
    for i in items:
        ...
        product.save()
    avoir.delete()
    return redirect('product:supplierinfo', id=suppid)
def printavoirsupp(request):
    id=request.GET.get('id')
    avoir=Avoirsupp.objects.get(pk=id)
    items=json.loads(avoir.items)
    ctx={
        'title':f'Avoir fournisseur {avoir.receipt_no}',
        'avoir':avoir,
        'items':items,
        'today':timezone.now()
    }
    return render(request, 'products/printavoirsupp.html', ctx)
