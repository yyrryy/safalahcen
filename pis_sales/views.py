...
def facture(request, pk):
    # we dintr create facture, we just
...
class InvoicesList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'title':'Liste Bons',
            'bons':SalesHistory.objects.filter(hasmultiple=False).order_by('-id'),
        })
        return context
...
class UpdateInvoiceView(FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'invoice': invoice,
            'customers':Customer.objects.exclude(pk=invoice.customer.id),
            'categories':Category.objects.filter(children__isnull=True).order_by('name')
        })
        return context
...
class UpdateInvoiceAPIView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if invoice.customer:
            ...
            newcustomer=Customer.objects.get(id=clientid)
            if newcustomer==customer:
                customer.rest=float(customer.rest)-float(oldtotal)+float(newtotal)
                customer.save()
            else:
                customer.rest=float(customer.rest)-float(oldtotal)
                customer.save()
                newcustomer.rest=float(newcustomer.rest)+float(newtotal)
                newcustomer.save()
        # add new total
        ...
        with transaction.atomic():
            ...
            for i in items:
                ...
                clientprice=Clientprice.objects.filter(client=newcustomer, product=product).first()
                if clientprice:
                    clientprice.price=float(i.get('price'))
                    clientprice.qty=float(i.get('qty'))
                    clientprice.save()
                else:
                    Clientprice.objects.create(client=newcustomer, product=product, price=float(i.get('price')), qty=float(i.get('qty')))
                # try:
